	     ...
	     if(url.contains("index.do")) {
	          ...
	          rd.forward(request, response);
				...
				int lastPage = pager[5];
				 ...
				 List<ProductDTO> arr = productDao.getListAll(startRecord, lastRecord);
	    	 ...
	    	 int result = dao.setInsert(dto);
				...
				int lastPage = pager[5];
				 ...
				 request.setAttribute("lastPage", lastPage);
				 request.setAttribute("jj", jj);
				 System.out.println("저기" + cookNo);
				 List<MallDTO> arr = dao.getListAll(cookNo, startRecord, lastRecord);
				 System.out.println("여기" + cookNo);
				 request.setAttribute("arr", arr);
