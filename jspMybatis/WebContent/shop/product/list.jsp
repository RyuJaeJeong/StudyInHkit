	...
	<c:forEach var="dto" items="${arr}">
		<tr>
			...
			<td>
				...
				<c:if test="${dto.product_img != '-,-,-' }">
					<c:set var = "temp1" value="${fn:split(dto.product_img,',')[0]}"></c:set>	<!-- 1번 사진 이름  -->
					<c:set var = "temp2" value="${fn:split(temp1,'|')[0]}"></c:set> <!-- 1번 사진 유저가 업로드한 이름  -->
					<c:set var = "temp3" value="${fn:split(temp1,'|')[1]}"></c:set> <!-- 1번 사진 유저가 실제 이름  -->
